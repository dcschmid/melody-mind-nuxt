...
export const getExpertResponse = (confidenceLevel: ConfidenceLevel, locale: string): string => {
  ...
  if (!responseCache.has(cacheKey)) {
    const responses = expertResponsesByLocale[locale]?.[confidenceLevel] ??
...
export const generateExpertOpinion = (
  ...
  const confidenceLevel: ConfidenceLevel =
    random < CONFIDENCE_PROBABILITIES.HIGH_THRESHOLD ? 'high' :
    random < CONFIDENCE_PROBABILITIES.MEDIUM_THRESHOLD ? 'medium' : 'low';

  const isCorrectAnswer =
    confidenceLevel === 'high' ? true :
    confidenceLevel === 'medium' ? Math.random() < CONFIDENCE_PROBABILITIES.MEDIUM_CORRECT_PROBABILITY :
