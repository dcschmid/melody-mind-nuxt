// server/api/highscore/category.post.ts

import { useTurso } from "../../../lib/turso";
import { nanoid } from "nanoid";

export default defineEventHandler(async (event) => {
  const { userId, category, score, language } = await readBody(event);
  const client = useTurso();

  try {
    // Validiere den Score
    const numericScore = Number(score);
    if (!Number.isFinite(numericScore)) {
      throw createError({
        statusCode: 400,
        message: "Invalid score value",
      });
    }

    // Check if a better score exists
    const {
      rows: [existingScore],
    } = await client.execute({
      sql: `SELECT score
            FROM highscore_per_category
            WHERE user_id = ? AND category = ? AND language = ?`,
      args: [userId, category, language],
    });

    // Validiere den existierenden Score und berechne die Summe
    const existingScoreNum = existingScore?.score ? Number(existingScore.score) : 0;
    if (!Number.isFinite(existingScoreNum)) {
      throw createError({
        statusCode: 500,
        message: "Invalid existing score in database",
      });
    }

    const totalScore = existingScoreNum + numericScore;

    if (existingScore) {
      await client.execute({
        sql: `UPDATE highscore_per_category
              SET score = ?
              WHERE user_id = ? AND category = ? AND language = ?`,
        args: [totalScore, userId, category, language],
      });
    } else {
      await client.execute({
        sql: `INSERT INTO highscore_per_category (id, user_id, category, score, language)
              VALUES (?, ?, ?, ?, ?)`,
        args: [nanoid(), userId, category, numericScore, language],
      });
    }

    return { success: true };
  } catch (error: any) {
    console.error("Database error:", error);
    throw createError({
      statusCode: error.statusCode || 500,
      message: error.message || "Database error",
    });
  }
});
